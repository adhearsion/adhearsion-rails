...
describe AhnRails::Plugin::Service do

  describe "while initializing" do
    before do
      Adhearsion.config.ahn_rails.path = "/path/to/rails/app"
      stub_file_checking_methods!
    end

    after do
      defined?(Activerecord) and Object.send(:remove_const, :"ActiveRecord")
      Adhearsion.config.ahn_rails.path = ""
      Adhearsion.config.ahn_rails.environment = :development
    end

    let :environment_rb do
      "#{Adhearsion.config.ahn_rails.path}/config/environment.rb"
    end
    
    it "should load the config/environment.rb file within the rails_root path" do
      AhnRails::Plugin::Service.should_receive(:load_env_file).once.with(environment_rb).and_return true
    ...
    it "should raise an exception if environment is nil" do
      Adhearsion.config.ahn_rails.environment = nil
  ...
  describe "while initializing with invalid rails path" do
    ...
    it "should raise an exception" do
      Adhearsion.config.ahn_rails.path = "/path/to/rails/app"
      lambda { Adhearsion::Plugin.load }.should raise_error "Unable to load Rails. Invalid rails root path: #{Adhearsion.config.ahn_rails.path.inspect}."
    end
  end
